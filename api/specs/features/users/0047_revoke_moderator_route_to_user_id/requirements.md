# 0047 · revoke_moderator_route_to_user_id — Requirements
...
- **F1.** `PATCH /api/v1/users/{user_id}/revoke-moderator` accepts `user_id: int`
  as a path parameter and, on success, returns HTTP 200 with a
  `RevokeModeratorResponse` body carrying `id`, `name`, `username`, `email`,
  `profile_image_url`, `tier_id`, and `is_moderator: false`.
...
- **F5.** `RevokeModeratorCommand` carries `target_user_id: int` and
  `requester_is_superuser: bool`; the router populates them from the `user_id`
  path param and `current_superuser["is_superuser"]` respectively.
- **F6.** The use-case raises `ForbiddenDomainError("Superuser privilege
  ...
  call is made.
- **F7.** The use-case calls `RevokeModeratorPort.get_by_id(command.target_user_id)`
  and raises `NotFoundDomainError("User not found")` when the result is `None`.
- **F8.** The use-case raises `DuplicateValueDomainError("User is not a
  moderator")` when the entity returned by `get_by_id` has `is_moderator == False`.
- **F9.** The use-case calls
  `RevokeModeratorPort.revoke(command.target_user_id)` only after the superuser
  check (F6), existence check (F7), and is-a-moderator check (F8) all pass, and
  returns the resulting `RevokedUser`.
- **F10.** `RevokeModeratorAdapter.get_by_id(user_id: int)` queries `User`
  filtering by `User.id == user_id` and `User.is_deleted == False`; it returns a
  `RevokedUser` populated from the row when one is found, or `None` when no row
  matches.
- **F11.** `RevokeModeratorAdapter.revoke(target_user_id: int)` executes
  ...
  returns the refreshed `RevokedUser`.
...
- **N1.** The use-case (`RevokeModeratorUseCase`) is a class with a single
...
- **N2.** The adapter catches only business-meaningful infrastructure exceptions;
  `revoke` has no unique-constraint or business-meaningful FK path, so no
...
- **N3.** `RevokeModeratorAdapter` explicitly inherits from `RevokeModeratorPort`
  (`class RevokeModeratorAdapter(RevokeModeratorPort):`). Per
  `agent_docs/architecture.md` § Adapter pattern.
- **N4.** `RevokeModeratorPort` carries the `@runtime_checkable` decorator and
...
- Updating shared test fixtures used by other slices — only files under
  `tests/features/users/0016_revoke_moderator/` are changed.
...
| F8 | use-case unit test (target.is_moderator=False → DuplicateValueDomainError) |
| F9 | use-case unit test (happy path → revoke called with target_user_id) |
| F10 | adapter unit test (row found vs. not found / is_deleted cases) |
| F11 | adapter unit test (revoke UPDATE assertions + refreshed entity) |
| F12 | endpoint integration test (404 case) |
| F13 | endpoint integration test (409 case); outside-in test (state check) |
