...
_CMD = DeleteTierCommand(name="silver")
...
async def test_not_found_raises_and_delete_not_called() -> None:
    """F2 — port.get returns None → NotFoundDomainError; port.delete never called."""
...
async def test_happy_path_calls_delete_and_returns_none() -> None:
    """F3 — port.get returns TierItem → port.delete called; use-case returns None."""
    port = _make_port()
    use_case = DeleteTierUseCase(port=port)  # type: ignore[arg-type]
    result = await use_case(_CMD)
    port.delete.assert_called_once_with("silver")
