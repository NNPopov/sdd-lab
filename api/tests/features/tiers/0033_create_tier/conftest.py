# FEATURE: create_tier — outside-in test fixtures.
...
async def async_client(oit_engine) -> AsyncClient:
    ...
    from the old write_tier handler alike — go through the same test transaction.
...
async def _clean_test_tiers(async_client: AsyncClient) -> None:
    ...
    Rows created by the old write_tier handler in a previous (interrupted) red-state
    ...
    async with _di_container.session_factory()() as session:
        await session.execute(text("DELETE FROM tier WHERE name = 'gold'"))
