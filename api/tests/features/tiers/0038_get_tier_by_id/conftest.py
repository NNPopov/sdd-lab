# FEATURE: get_tier_by_id — outside-in test fixtures.
...
async def async_client(oit_engine) -> AsyncClient:
    ...
    join_transaction_mode="create_savepoint" means every session.commit() inside
    an adapter creates a SAVEPOINT, not a real commit. Rolling back the outer
    transaction at teardown removes all test data.
    """
