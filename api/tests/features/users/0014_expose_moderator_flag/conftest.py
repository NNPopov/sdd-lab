# FEATURE: expose_moderator_flag — outside-in test fixtures.
...
async def seeded_alice(async_client: AsyncClient) -> dict:
    """Insert one non-moderator User row via the overridden session factory.

    Returns the full user dict (all UserMeRead fields) so it can be used as
    the get_current_user dependency override in tests that call GET /user/me/.
    """
