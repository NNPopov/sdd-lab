# FEATURE: update_user_tier_route_to_user_id — outside-in test fixtures.
#
# Fat-handler migration slice: patch_user_tier injects the session via
...
# free function and any container-wired code (e.g. the slice 0048 get_user_tier
# use-case used to verify the persisted change) share the same savepoint
...
@pytest_asyncio.fixture()
async def seeded_user_and_tier(async_client: AsyncClient) -> dict:
    """Insert a tier and a user with no tier assigned (tier_id is None initially).

    The slice under test assigns the tier via PATCH; the user therefore starts
    without one so the update is observable.
    """
    ...
    async with _di_container.session_factory()() as session:
        ...
        user = User(
            name="UT Target",
            username="ut050target",
            email="ut050target@example.com",
            hashed_password="fake_hashed_password",
            profile_image_url="https://www.profileimageurl.com",
        )
        session.add(user)
