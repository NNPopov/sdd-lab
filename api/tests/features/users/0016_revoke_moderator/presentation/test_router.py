...
#
# Covers: F1, F2, F13, F14, F12.
import pytest
from httpx import AsyncClient

pytestmark = pytest.mark.asyncio

_ENDPOINT = "/api/v1/users/{username}/revoke-moderator"
...
async def test_revoke_moderator_returns_403_for_non_superuser(
    ...
    """F2 — authenticated user is not a superuser; 403 returned."""
    ...
    try:
        response = await async_client.patch(_ENDPOINT.format(username=seeded_moderator_user["username"]))
    finally:
...
# ── F13: user not found → 404 ────────────────────────────────────────────────


async def test_revoke_moderator_returns_404_for_unknown_username(
    async_client: AsyncClient,
    seeded_superuser: dict,
) -> None:
    """F13 — target username does not exist; 404 returned."""
    from app.features.users.dependencies import get_current_user
    from app.main import app as _fastapi_app
...
async def test_revoke_moderator_returns_409_when_not_a_moderator(
    ...
    """F14 — target is not a moderator; 409 returned."""
    from app.features.users.dependencies import get_current_user
    from app.main import app as _fastapi_app

    _fastapi_app.dependency_overrides[get_current_user] = lambda: seeded_superuser
    try:
        response = await async_client.patch(_ENDPOINT.format(username=seeded_regular_user["username"]))
    finally:
        del _fastapi_app.dependency_overrides[get_current_user]
...
async def test_revoke_moderator_returns_200_on_success(
    ...
    """F12 — superuser revokes moderator; 200 with is_moderator=false."""
    from app.features.users.dependencies import get_current_user
    from app.main import app as _fastapi_app
