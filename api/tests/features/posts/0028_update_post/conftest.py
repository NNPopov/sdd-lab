# FEATURE: update_post — outside-in test fixtures.
...
async def async_client(oit_engine) -> AsyncClient:
    ...
    legacy patch_post handler during the red phase) and the container's
    session_factory (used by the new update_post adapter in the green phase)
...
@pytest_asyncio.fixture()
async def up28_alice(async_client: AsyncClient) -> dict:
...
@pytest_asyncio.fixture()
async def up28_bob(async_client: AsyncClient) -> dict:
...
@pytest_asyncio.fixture()
async def up28_alice_post(up28_alice: dict, async_client: AsyncClient) -> dict:
