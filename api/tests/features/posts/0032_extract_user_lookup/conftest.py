# FEATURE: extract_user_lookup — test fixtures.
...
async def async_client(oit_engine) -> AsyncClient:
    ...
    Overrides the DI container's session_factory so that all adapter calls within the
    test use the same connection and can be rolled back atomically at teardown.
    """
...
@pytest_asyncio.fixture()
async def el32_alice(async_client: AsyncClient) -> dict:
    """Insert one active User row for 'el32alice'; return id and username."""
