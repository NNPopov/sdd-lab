# FEATURE: migrate_erase_post_route_username_to_user_id — outside-in test fixtures.
...
async def async_client(oit_engine) -> AsyncClient:
    ...
    after delete is the real DB state, not a stale cache hit). Overrides both
    async_get_db and the container's session_factory so every code path sees the
    seeded data inside the test transaction.
    """
...
@pytest_asyncio.fixture()
async def ep57_alice(async_client: AsyncClient) -> dict:
...
@pytest_asyncio.fixture()
async def ep57_bob(async_client: AsyncClient) -> dict:
...
@pytest_asyncio.fixture()
async def ep57_alice_post(ep57_alice: dict, async_client: AsyncClient) -> dict:
    """Insert an approved Post owned by alice; return its dict.

    Status 'approved' ensures the verification GET in scenario 1 returns 200
    before deletion (approved posts are publicly visible per the get_post slice),
    making the post-deletion 404 unambiguously caused by the soft-delete.
    """
    ...
    async with _di_container.session_factory()() as session:
        post = Post(
            created_by_user_id=ep57_alice["id"],
