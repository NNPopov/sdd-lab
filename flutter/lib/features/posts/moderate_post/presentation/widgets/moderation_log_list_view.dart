...
class ModerationLogListView extends StatelessWidget {
  const ModerationLogListView({super.key});
    ...
    return BlocBuilder<ModerationLogCubit, ModerationLogState>(
      builder: (context, state) => switch (state) {
        ...
        ModerationLogLoaded(:final entries) =>
          entries.isEmpty
              ? Center(child: Text(t.posts.moderatePost.log.empty))
              : ListView.builder(
        ...
        ModerationLogError() => Center(
          child: Text(t.posts.moderatePost.log.loadError),
        ),
      },
    );
  }
