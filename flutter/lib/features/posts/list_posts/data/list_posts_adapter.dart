...
@LazySingleton(as: ListPostsPort)
class ListPostsAdapter implements ListPostsPort {
  ListPostsAdapter(this._api, this._logger);
  ...
  Future<Either<Failure, PaginatedPosts>> call({
    ...
    try {
      try {
        final dto = await _api.getPosts(page: page, perPage: perPage);
        return Right(
          PaginatedPosts(
            items: dto.items
                .map(
      ...
      _logger.error(
        'ListPostsAdapter.call failed unexpectedly',
        error: e,
        stackTrace: st,
      );
