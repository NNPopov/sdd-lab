...
void main() {
  ...
  setUp(() {
    ...
    // DeletePostButton (visible when isAuthor) pulls its cubit from getIt.
    when(
  ...
  // ── Scenario 1: author sees the correct chip label per post status ──────────

  testWidgets(
    'author sees Pending Review chip when post status is pendingReview',
    (tester) async {
      await tester.pumpWidget(
        buildSubject(
          postState: PostDetailsState.loaded(
            post: _fakePost(status: PostStatus.pendingReview),
          ),
  ...
  testWidgets(
    'author sees Approved chip when post status is approved',
    (tester) async {
      await tester.pumpWidget(
        buildSubject(
          postState: PostDetailsState.loaded(
  ...
  testWidgets(
    'author sees Changes Requested chip when post status is changesRequested',
    (tester) async {
      await tester.pumpWidget(
        buildSubject(
          postState: PostDetailsState.loaded(
  ...
  // ── Scenario 2: non-author and unauthenticated see no chip ─────────────────

  testWidgets(
    'authenticated non-author does not see status chip',
    (tester) async {
      await tester.pumpWidget(
        buildSubject(
  ...
  testWidgets(
    'unauthenticated user does not see status chip',
    (tester) async {
      await tester.pumpWidget(
        buildSubject(
          postState: PostDetailsState.loaded(post: _fakePost()),
